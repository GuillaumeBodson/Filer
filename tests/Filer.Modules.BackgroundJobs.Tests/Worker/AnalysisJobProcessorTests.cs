...
/// <summary>
/// The claim/dispatch core shared by the polling worker and the RabbitMQ consumer
/// (ADR-008), against fake seams: a claimed job is dispatched exactly once and its
/// outcome recorded; failures retry with exponential backoff until the attempt
/// limit, then fail terminally; a deleted document cancels the job; shutdown
/// mid-flight releases the job so no work is lost (06, 12-testing-strategy.md).
/// </summary>
public sealed class AnalysisJobProcessorTests
{
    ...
    public AnalysisJobProcessorTests() =>
    ...
    private AnalysisJobProcessor CreateSut(
        BackgroundJobsOptions? options = null,
        ILogger<AnalysisJobProcessor>? logger = null)
    {
        // Real DI scopes so the scope-per-claim behaviour is exercised.
        ...
        return new AnalysisJobProcessor(
            ...
            logger ?? NullLogger<AnalysisJobProcessor>.Instance);
    }
    ...
    public async Task ProcessNextAsync_WhenHandlerSucceeds_MarksJobSucceededWithTheResultPayload()
    {
        ...
        payload.Should().Be(resultJson, "the processor persists the handler's serialized result verbatim");
    ...
    public async Task ProcessNextAsync_WhenHandlerThrowsBeforeTheAttemptLimit_SchedulesARetry()
    {
        ...
        // The processor survives a throwing handler: the attempt is recorded as a
    ...
    {
        // The retry state machine end to end at the processor's level: attempt 1
        ...
        AnalysisJobProcessor sut = CreateSut();
    ...
    {
        // The claim stamps AnalysisJob.Provider (#163): the processor must hand
        // the store the name of the handler that will run the attempt.
    ...
    public async Task ProcessNextAsync_LogsTheClaimWithMessageTemplateAndJobScope()
    {
        ...
        _handler
            .Setup(h => h.HandleAsync(job, It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Success<string?>(null));
        var logger = new FakeLogger<AnalysisJobProcessor>();
